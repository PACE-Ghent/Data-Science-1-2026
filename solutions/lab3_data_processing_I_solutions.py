...
def _(DATA_DIR, pd):
    ...
    tests_raw_shape = tests_raw.shape
...
@app.cell
def _(athletes_raw, younger_than_16):
    forwards_u16_raw = athletes_raw.loc[(athletes_raw["position"] == "Forward") & younger_than_16]
...
@app.cell
def _(tests_raw):
    suspicious_30m_raw = tests_raw.loc[tests_raw["sprint_30m_s"] > 20].sort_values(
...
def _(mo, position_counts_raw):
    mo.md(
        f"`value_counts()` on the raw `position` column returns "
...
@app.cell
def _(athletes_raw, weight_kg_numeric):
    athletes_with_bmi = athletes_raw.copy()
    athletes_with_bmi["bmi"] = weight_kg_numeric / (athletes_with_bmi["height_cm"] / 100) ** 2
...
def _(REFERENCE_DATE, athletes_raw, pd):
    ...
    athletes_with_age["age"] = (REFERENCE_DATE - pd.to_datetime(athletes_with_age["birthdate"])).dt.days // 365
...
@app.cell
def _(athletes_raw, position_clean):
    athletes_step1 = athletes_raw.copy()
    athletes_step1["position"] = position_clean
...
def _(fix_sprint_units, tests_raw):
    sprint_30m_fixed = fix_sprint_units(tests_raw["sprint_30m_s"])
...
@app.cell
def _(athletes_step1):
    athletes_deduped = athletes_step1.drop_duplicates(subset=["name", "birthdate"], keep="first")
...
@app.cell
def _(athletes_clean, tests_clean):
