...
@app.cell
def _(athletes, px):
    weight_histogram = px.histogram(
        athletes,
        x="weight_kg",
        title="Distribution of athlete weight",
...
def _(POSITION_COLORS, combined, px):
    yoyo_by_age = px.box(
        ...
        color_discrete_map=POSITION_COLORS,
        facet_col="gender",
        title="Yo-Yo level by position and gender",
    )
    return (yoyo_by_age,)
...
@app.cell
def _(POSITION_COLORS, combined, dashboard_selector, px):
    dashboard_chart_1 = px.histogram(
        combined, x="sprint_30m_s", title="30 m sprint time - all athletes",
        labels={"sprint_30m_s": "Sprint time (s)"},
    )
    dashboard_chart_2 = px.box(
        combined, x="position", y="cmj_height_cm", color="position", color_discrete_map=POSITION_COLORS,
        title="Jump height by position",
    )
    dashboard_chart_3 = px.bar(
        combined.loc[combined["position"] == dashboard_selector.value]
        .groupby("test_moment", as_index=False)["sprint_30m_s"].mean(),
        x="test_moment", y="sprint_30m_s",
        title=f"Average sprint time across the season - {dashboard_selector.value}",
        labels={"sprint_30m_s": "Sprint time (s)", "test_moment": "Test moment"},
    )
    dashboard_charts = [dashboard_chart_1, dashboard_chart_2, dashboard_chart_3]
    return (dashboard_charts,)


@app.cell
def _(dashboard_charts, dashboard_selector, mo):
    mo.vstack([dashboard_selector, mo.hstack(dashboard_charts, wrap=True)])
    return
